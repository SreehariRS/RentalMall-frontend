...
function Input(props: InputProps) {
    const { id, label, type = "text", disabled, formatPrice, register, required, errors } = props;

    return (
        <div className="w-full relative">
            {formatPrice && <BiDollar size={24} className="text bg-neutral-700 absolute top-5 left-2" />}
            <input
                id={id}
                disabled={disabled}
