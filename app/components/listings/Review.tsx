...
interface ReviewProps {
  ...
  onReviewChange?: () => void;
  listingId?: string;
  isHost?: boolean; // Determines if the current user is the host
}

function Review({ reviews, currentUser, onReviewChange, listingId, isHost }: ReviewProps) {
  ...
  const handleReplySubmit = useCallback(
    (reviewId: string) => {
      if (!currentUser || !listingId || !replyContent[reviewId] || !isHost) return;

      axios
        .post("/api/reviews", {
          listingId,
          responseToReviewId: reviewId,
    ...
    [currentUser, listingId, replyContent, onReviewChange, isHost]
  );
                ...
                <div className="mx-4 flex-1">
                  <div className="bg-gray-200 h-2 rounded-full">
                    <div
                      ...
                      style={{
                        width: `${ratingCounts[star] ? (ratingCounts[star] / reviews.length) * 100 : 0}%`,
                      }}
                    ></div>
                  </div>
                </div>
            ...
            <div className="p-4">
              ...
              {review.responses.length > 0 && (
                <div className="mt-4 border-t pt-4">
                  <h4 className="text-sm font-semibold text-gray-800 mb-2">Host Response:</h4>
                  {review.responses.map((response) => (
                    <div key={response.id} className="mb-2">
                      <div className="flex items-center space-x-2">
                        <Avatar src={null} size={24} className="border-2 border-white" />
                        <span className="text-sm font-medium text-gray-800">{response.author}</span>
              ...
              {isHost && currentUser && (
                <div className="mt-4">
                  ...
                  <button
                    ...
                    disabled={!replyContent[review.id]}
                  >
                    Submit Reply
                  </button>
                </div>
