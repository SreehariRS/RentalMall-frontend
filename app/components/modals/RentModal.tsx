...
function RentModal() {
      ...
      <div className="grid grid-cols-2 gap-3 max-h-[50vh] overflow-y-auto">
        {categories.map((item) => (
          <div key={item.label} className="col-span-1">
            <CategoryInput
              onClick={(category) => setCustomerValue("category", category)}
  ...
  if (step === STEPS.LOCATION) {
    bodyContent = (
      <div className="flex flex-col gap-8">
        ...
        <CountrySelect
          value={location}
          onChange={(value) => setCustomerValue("location", value)}
        />
  ...
  if (step === STEPS.INFO) {
    const skipInfoCategories = ["car", "Bike", "cycle", "Tools"];

    if (skipInfoCategories.includes(category)) {
      onNext();
      return null;
    }
    ...
    bodyContent = (
      <div className="flex flex-col gap-3">
        ...
        <Counter
          ...
          value={guestCount}
          onChange={(value) => setCustomerValue("guestCount", value)}
        />
        <hr />
        <Counter
          title="Rooms"
          ...
          value={roomCount}
          onChange={(value) => setCustomerValue("roomCount", value)}
        />
      </div>
    );
  }
